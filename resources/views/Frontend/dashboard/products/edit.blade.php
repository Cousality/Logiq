...
<head>
    ...
    <title>Edit Product - LOGIQ Admin</title>
...
<body>
    ...
    <header class="dashboard-header">
        ...
        <p>Update the details for <strong>{{ $product->productName }}</strong>.</p>
    </header>
    ...
    <main class="management-container">
        ...
        <div class="settings-card">
            ...
            <form action="{{ route('admin.products.update', $product->productID) }}"
                  method="POST"
                    ...
                    <div class="form-group">
                        ...
                        <select id="productCategory" name="productCategory" required>
                            @foreach($categories as $cat)
                                <option value="{{ $cat }}"
                                    {{ old('productCategory', $product->productCategory) === $cat ? 'selected' : '' }}>
                                    {{ $cat }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select id="productDifficulty" name="productDifficulty" required>
                            @foreach($difficulties as $diff)
                                <option value="{{ $diff }}"
                                    {{ old('productDifficulty', $product->productDifficulty) === $diff ? 'selected' : '' }}>
                                    {{ ucfirst($diff) }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    <select id="productStatus" name="productStatus" required>
                        @foreach($statuses as $status)
                            <option value="{{ $status }}"
                                {{ old('productStatus', $product->productStatus) === $status ? 'selected' : '' }}>
                                {{ ucfirst($status) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <p class="image-hint">JPG, PNG or WebP · max 2 MB · leave blank to keep existing image</p>
                    @error('productImage')
                        <span class="error">{{ $message }}</span>
                    @enderror
                </div>
